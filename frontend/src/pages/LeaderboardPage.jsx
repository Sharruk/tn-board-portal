...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    document.title = 'Contributors & Leaderboard | TN Board Portal'
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-base text-gray-600 max-w-2xl mx-auto">
            Honoring students, teachers, and educators who share approved Tamil Nadu State Board question papers and answer keys.
          </p>
          <div className="pt-2">
            <Link
              to="/submit-material"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
            ...
            {top3[1] && (
              <div
                ...
                className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col items-center text-center relative order-2 md:order-1 cursor-pointer hover:border-blue-300 hover:shadow-md transition"
              >
                ...
                <div className="flex flex-wrap gap-1 justify-center my-2">
                  {top3[1].badges?.slice(0, 2).map(b => (
                    <span key={b} className="text-[10px] bg-amber-50 text-amber-800 border border-amber-200 px-2 py-0.5 rounded-full font-semibold">
                      {b}
                    </span>
                  ))}
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2 w-full pt-3 border-t border-gray-100 text-xs">
                  <div className="bg-gray-50 rounded-lg p-2">
                    <p className="text-gray-500 font-medium">Approved</p>
                    <p className="text-base font-bold text-emerald-600">{top3[1].approved_count ?? top3[1].accepted_contributions}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-2">
            ...
            {top3[0] && (
              <div
                ...
                className="bg-gradient-to-b from-amber-50 to-white rounded-2xl border-2 border-amber-300 p-6 shadow-md flex flex-col items-center text-center relative order-1 md:order-2 -mt-2 cursor-pointer hover:shadow-lg transition"
              >
                ...
                <div className="flex flex-wrap gap-1 justify-center my-2">
                  {top3[0].badges?.slice(0, 2).map(b => (
                    <span key={b} className="text-[10px] bg-amber-100 text-amber-800 border border-amber-300 px-2 py-0.5 rounded-full font-bold">
                      {b}
                    </span>
                  ))}
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2 w-full pt-3 border-t border-amber-100 text-xs">
                  <div className="bg-amber-50/70 rounded-lg p-2">
                    <p className="text-amber-700 font-medium">Approved</p>
                    <p className="text-lg font-extrabold text-emerald-600">{top3[0].approved_count ?? top3[0].accepted_contributions}</p>
                  </div>
                  <div className="bg-amber-50/70 rounded-lg p-2">
            ...
            {top3[2] && (
              <div
                ...
                className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col items-center text-center relative order-3 cursor-pointer hover:border-blue-300 hover:shadow-md transition"
              >
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 w-full pt-3 border-t border-gray-100 text-xs">
                  <div className="bg-gray-50 rounded-lg p-2">
                    <p className="text-gray-500 font-medium">Approved</p>
                    <p className="text-base font-bold text-emerald-600">{top3[2].approved_count ?? top3[2].accepted_contributions}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-2">
                    <p className="text-gray-500 font-medium">Acceptance</p>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-gray-50/80 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    ...
                    <th className="py-3.5 px-4 text-center">Approved Files</th>
                    <th className="py-3.5 px-4 text-right pr-6">Acceptance Rate</th>
                    <th className="py-3.5 px-4">Badges &amp; Recent Material</th>
                  </tr>
                </thead>
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex items-center gap-2.5">
                            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-600 text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-sm overflow-hidden">
                              {item.avatar_url ? (
                            ...
                            <div>
                              <p className="font-semibold text-gray-900 hover:text-blue-600 transition-colors">{item.contributor_name}</p>
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {item.badges?.map(b => (
                              <span key={b} className="text-[10px] px-2 py-0.5 bg-amber-50 text-amber-800 border border-amber-200 rounded-full font-medium">
                                🏅 {b}
                              </span>
                            ))}
                            {item.recent_contributions?.length > 0 && (
                              <span className="text-[11px] text-gray-400 truncate max-w-[200px]" title={item.recent_contributions.join(', ')}>
                                Recent: {item.recent_contributions[0]}
                              </span>
                            )}
                          </div>
                        </td>
